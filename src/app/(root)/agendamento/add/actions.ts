...
export async function getTipo() {
	...
	const { data, error } = await supabase.rpc("func_tipoagendamento_sel");
...
export async function getSituacao() {
	...
	const { data, error } = await supabase.rpc("func_situacao_sel");
...
export async function add(form: FormData) {
	...
	if (_cep == null || _cep?.toString().trim() == "") {
		...
		const { data, error } = await supabase.rpc("proc_agendamento_ins", params);

		return { sucesso: "Agendamento adicionado com sucesso." };
	}
}
